...
//    }
    ...
    public function get_settings($cache_key = null)
    {
        $widget_settings = Cache::remember($cache_key,600,function(){
            return PageBuilder::find($this->args['id']);
        });
        $widget_data = !empty($this->args['id']) ? $widget_settings : [];
        $widget_data = !empty($widget_data) ? unserialize($widget_settings->addon_settings,['class' => false]) : [];
        return $widget_data;
    }
